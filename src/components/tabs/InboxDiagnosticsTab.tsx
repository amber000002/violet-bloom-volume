    ...
    <div className="space-y-6">
      ...
      {activeReport === "analysis" && (diagnostics?.analysisReport || journeyAnalysis) && (
        ...
        >
          ...
          {/* ============= SECTION 1: KPI Cards + Grand Totals (Combined / Campaigns / Journeys) ============= */}
          {(() => {
            ...
            const campaignTotals = sumCampaignProviderAggregates(campaignAggs);
            const journeyTotals = sumJourneyProviderAggregates(journeyAggs);
            const combinedSent = campaignTotals.totalSent + journeyTotals.totalSent;
            const combinedDelivered = campaignTotals.totalDelivered + journeyTotals.totalDelivered;
            const combinedViewed = campaignTotals.uniqueViewed + journeyTotals.uniqueViewed;
            const combinedClicked = campaignTotals.uniqueClicked + journeyTotals.uniqueClicked;
            const combinedUnsubs = campaignTotals.unsubscribes + journeyTotals.unsubscribes;
            const denom = combinedDelivered > 0 ? combinedDelivered : combinedSent;
            const openPct = denom > 0 ? (combinedViewed / denom) * 100 : 0;
            const clickPct = denom > 0 ? (combinedClicked / denom) * 100 : 0;
            const unsubPct = denom > 0 ? (combinedUnsubs / denom) * 100 : 0;

            return (
              <CollapsibleSection
                title="Campaign Overview (KPI)"
                icon={<BarChart3 className="w-5 h-5 text-primary" />}
                isOpen={expandedSections.provider}
                onToggle={() => toggleSection("provider")}
              >
                {/* KPI cards — Combined totals across campaign + journey */}
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Total Sent</div>
                    <div className="text-lg font-semibold">{formatNumber(combinedSent)}</div>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Total Delivered</div>
                    <div className="text-lg font-semibold">{formatNumber(combinedDelivered)}</div>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Unique Viewed</div>
                    <div className="text-lg font-semibold">{formatNumber(combinedViewed)}</div>
                    <div className="text-xs text-muted-foreground">{formatPercent(openPct)}</div>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Unique Clicked</div>
                    <div className="text-lg font-semibold">{formatNumber(combinedClicked)}</div>
                    <div className="text-xs text-muted-foreground">{formatPercent(clickPct)}</div>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Unsubscribes</div>
                    <div className="text-lg font-semibold">{formatNumber(combinedUnsubs)}</div>
                    <div className="text-xs text-muted-foreground">{formatPercent(unsubPct)}</div>
                  </div>
                  <div className="rounded-xl border border-border/60 bg-muted/30 p-3">
                    <div className="text-xs text-muted-foreground">Sources</div>
                    <div className="text-lg font-semibold">
                      {hasCampaigns && hasJourneys ? "Campaign + Journey" : hasCampaigns ? "Campaign" : "Journey"}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {denom === combinedDelivered ? "Denom: Delivered" : "Denom: Sent"}
                    </div>
                  </div>
                </div>

                {/* Combined sub-table (only when BOTH datasets present) */}
                {hasCampaigns && hasJourneys && (
                  <div className="mb-5">
                    <div className="text-xs font-semibold text-muted-foreground mb-2">Combined Totals</div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          <tr className="border-b border-border/50 bg-muted/20 font-semibold">
                            <td className="py-2 px-3">All Sources</td>
                            <td className="text-right py-2 px-3">{formatNumber(combinedSent)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(combinedDelivered)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(combinedViewed)}</td>
                            <td className="text-right py-2 px-3"><ColoredPercent value={openPct} metricType="openRate" /></td>
                            <td className="text-right py-2 px-3">{formatNumber(combinedClicked)}</td>
                            <td className="text-right py-2 px-3"><ColoredPercent value={clickPct} metricType="clickRate" /></td>
                            <td className="text-right py-2 px-3">{formatNumber(combinedUnsubs)}</td>
                            <td className="text-right py-2 px-3"><ColoredPercent value={unsubPct} metricType="unsubscribeRate" /></td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                {/* Campaign totals sub-table */}
                {hasCampaigns && (
                  <div className="mb-5">
                    <div className="text-xs font-semibold text-muted-foreground mb-2">Campaign Totals</div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-border">
                        ...
                        <tbody>
                          <tr className="border-b border-border/50">
                            <td className="py-2 px-3">Campaigns</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.totalSent)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.totalDelivered)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.uniqueViewed)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.uniqueClicked)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.unsubscribes)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.hardBounces)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(campaignTotals.softBounces)}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                {/* Journey totals sub-table */}
                {hasJourneys && (
                  <div className="mb-2">
                    <div className="text-xs font-semibold text-muted-foreground mb-2">Journey Totals</div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-border">
                        ...
                        <tbody>
                          <tr className="border-b border-border/50">
                            <td className="py-2 px-3">Journeys</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.totalSent)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.totalDelivered)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.uniqueViewed)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.uniqueClicked)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.unsubscribes)}</td>
                            <td className="text-right py-2 px-3">{formatNumber(journeyTotals.errors)}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    <p className="text-xs text-muted-foreground mt-2">
                      Bounces are not tracked in journey exports — campaign-only metrics.
                    </p>
                  </div>
                )}
              </CollapsibleSection>
          ...
          {/* ============= SECTION 2: Provider Performance — Combined / Campaigns / Journeys ============= */}
          {(journeyAnalysis?.providerAggregates?.length || diagnostics?.analysisReport?.providerAggregates?.length) ? (
            <CollapsibleSection
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {combinedAggs.map((p, i) => (
                                <tr key={i} className="border-b border-border/50 hover:bg-muted/20">
                                  <td className="py-2 px-3 font-medium">{p.providerName}</td>
                                  <td className="text-right py-2 px-3">{formatNumber(p.totalSent)}</td>
                                  <td className="text-right py-2 px-3">{formatNumber(p.totalDelivered)}</td>
                    ...
                    {/* Journeys-only sub-table */}
                    {hasJourneys && (
                      <div>
                        <div className="text-xs font-semibold text-muted-foreground mb-2">Journeys</div>
                        <div className="overflow-x-auto">
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b border-border">
                            ...
                            <tbody>
                              {journeyAggs.map((p, i) => (
                                <tr key={i} className="border-b border-border/50 hover:bg-muted/20">
                                  ...
                                  <td className="text-right py-2 px-3">{formatNumber(p.unsubscribes)}</td>
                                  <td className="text-right py-2 px-3"><ColoredPercent value={p.unsubscribePercent} metricType="unsubscribeRate" /></td>
                                  <td className="text-right py-2 px-3">{formatNumber(p.errors)}</td>
                                  <td className="text-right py-2 px-3"><ColoredPercent value={p.errorPercent} metricType="bounceRate" /></td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
