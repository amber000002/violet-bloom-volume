...
import { ChevronDown, ChevronUp, CheckCircle2, AlertTriangle, Lightbulb, Sparkles, BookOpen, Download, Zap } from "lucide-react";
...
const sourceBadge = (source: string) => {
  const isInternal = source.includes("Internal");
  const isAINative = source.includes("AI-Native");
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
      isInternal ? "bg-emerald-500/10 text-emerald-400" : 
      isAINative ? "bg-purple-500/10 text-purple-400" :
...
const ExecutionDetailBlock: React.FC<{ detail: ExecutionDetail }> = ({ detail }) => {
  const fields = detail.fields || {};
  return (
    <div className="p-3 rounded-lg bg-muted/20 border border-border space-y-2">
      <div className="flex items-center gap-1.5">
        <Zap className="w-3 h-3 text-primary" />
        <span className="text-xs font-medium text-primary capitalize">{detail.channel}</span>
      </div>
      <p className="text-xs text-muted-foreground italic">{detail.trigger}</p>
      <div className="grid gap-1">
        {Object.entries(fields).map(([key, value]) => (
          <div key={key} className="flex gap-2 text-xs">
            <span className="text-muted-foreground font-medium capitalize min-w-[80px]">{key.replace(/_/g, " ")}:</span>
...
export const AugmentedUseCaseTable: React.FC<AugmentedUseCaseTableProps> = ({
  ...
  const byStage: Record<string, AugmentedUseCase[]> = {};
  for (const uc of useCases) {
    const stage = uc.lifecycle_stage;
    if (!byStage[stage]) {
      byStage[stage] = [];
      stageOrder.push(stage);
    }
    byStage[stage].push(uc);
  }

  const internalCount = useCases.filter(uc => uc.source.includes("Internal")).length;
  const nativeCount = useCases.filter(uc => uc.source.includes("AI-Native")).length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          {useCases.length} total • {internalCount} augmented internal • {nativeCount} AI-native expansions
        </div>
        ...
        <div key={stage} className="space-y-2">
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-xs text-muted-foreground">
              {byStage[stage].length} use cases
            </span>
          </div>
          <div className="rounded-xl border border-border overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[240px]">Use Case Title</TableHead>
                            ...
                            <td colSpan={5} className="p-0">
                              ...
                              >
                                <div className="p-5 space-y-4">
                                  ...
                                  {uc.execution_details && uc.execution_details.length > 0 && (
                                    <div>
                                      <span className="text-xs font-medium text-foreground mb-2 block">
                                        Channel-Specific Execution
                                      </span>
                                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {uc.execution_details.map((detail, i) => (
                                          <ExecutionDetailBlock key={i} detail={detail} />
                                        ))}
                                      </div>
                                    </div>
                                  )}
